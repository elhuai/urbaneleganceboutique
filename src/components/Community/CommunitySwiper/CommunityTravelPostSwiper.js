...
import './_CommunityTravelPostSwiper.scss';
...
export default function CommunityTravelPostSwiper (props) {
  const { comHomePageCard } = props;
  const [allTravelPost, setAllTravelPost] = useState([]);
  ...
  useEffect(() => {
    const fetchTravelPostData = async () => {
      const travelPostResult = await axios.get(`${API_URL}/communityHomePage/travelPost`);
      // setAllPost(result.data)
      console.log('-----------travelPostResult------------', travelPostResult.data);
      setAllTravelPost(travelPostResult.data);
    };
    fetchTravelPostData();
  }, []);
